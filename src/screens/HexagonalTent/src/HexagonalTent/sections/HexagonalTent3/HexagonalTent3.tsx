import { FrameHexagonalTent3 } from "./FrameHexagonalTent3/FrameHexagonalTent3";
import { FrameWrapperHexagonalTent3 } from "./FrameWrapperHexagonalTent3/FrameWrapperHexagonalTent3";
import { GroupWrapperHexagonalTent3 } from "./GroupWrapperHexagonalTent3/GroupWrapperHexagonalTent3";

export const HexagonalTent3 = (): JSX.Element => {
  return (
    <main className="flex flex-col w-full-[1440px] h-[3302px] bg-white">

<section   className="flex flex-col w-[1543px] h-[576px] gap-[40px] mx-auto px-12">
        <FrameWrapperHexagonalTent3 />
        

        <section className="w-[1440px] h-[334px] rounded-[30px] relative">

<FrameHexagonalTent3 />
<img
  className="absolute w-[356px] h-[857px] right-0 bottom-0 blur-[35px] object-cover z-0"
  alt="Blue water splashing"
/>
</section>

<section className="flex flex-col w-[1440px] h-[412px] gap-[45px] mt-80 mx-auto top-[2890px]">
        <GroupWrapperHexagonalTent3 />
      </section>

      </section>
    </main>
  );
};

